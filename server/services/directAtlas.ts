...
export async function connectDirectToAtlas(): Promise<Db> {
  ...
  for (let i = 0; i < connectionStrings.length; i++) {
    ...
    console.log(
      `🔄 Trying connection method ${i + 1}: ${uri.replace("Anilsharma123", "****")}`,
    );
    ...
    try {
      ...
      if (i === connectionStrings.length - 1) {
        throw new Error(
          `All connection methods failed. Last error: ${error.message}`,
        );
      }
    }
  }
...
export async function testAndInsertVehicle(): Promise<any> {
  try {
    ...
    await db
      .collection("vehicles")
      .deleteMany({ name: { $regex: /^Direct Test/ } });
