...
export async function connectToMongoDBAtlas(): Promise<{ db: Db | null, isConnected: boolean }> {
  try {
    ...
    for (let i = 0; i < connectionStrings.length; i++) {
      ...
      console.log(`🔄 Attempting connection ${i + 1}/${connectionStrings.length}...`);
      ...
      try {
        ...
        console.log(`❌ Connection attempt ${i + 1} failed:`, connectionError.message);
...
export async function forceMongoDBTest(): Promise<any> {
  ...
  // Clear test vehicles
  await db.collection('vehicles').deleteMany({ name: { $regex: /^Force Test/ } });
